...
form2.Hide;
 form3.tblCategory.First;		{встаем на первую запись}
While not form3.tblCategory.Eof do    {пока не конец БД}
  begin
     Form2.combobox1.Items.Add(form3.tblCategory['CATEGORY']);
     combobox1.Items.Add(form3.tblCategory['CATEGORY']);
     form3.tblCategory.Next;    	{переместиться на следующую запись}
  end;
  nach:=true;
 end;
end;
...
if form1.tblGibdd['CATEGORY']='А' then form2.combobox1.ItemIndex:=0;
if form1.tblGibdd['CATEGORY']='Б' then form2.combobox1.ItemIndex:=1;
if form1.tblGibdd['CATEGORY']='Д' then form2.combobox1.ItemIndex:=2;
if form1.tblGibdd['CATEGORY']='Е' then form2.combobox1.ItemIndex:=3;
if form1.tblGibdd['CATEGORY']='С' then form2.combobox1.ItemIndex:=4;
...
allchek:=false;
 ...
 begin
  if (edit7.text='') or (edit8.text='') or (edit1.text='') or (combobox1.Items[combobox1.ItemIndex]='Категория для поиска') or (edit10.text='') or (edit11.text='') then
  begin
...
begin
  ...
  begin
   if (edit7.text<>'') and (pos('Номер для поиска',edit7.text)=0) then
   tblGibdd.FindKey([edit7.Text])
   else showmessage('Вы не ввели фразу для поиска');
  end
  else
  begin
  showmessage('Вы не отметили поле Номера для поиска!');
  end;
end;
