...
begin
  ...
  Label5.Caption := 'Количество замен: ' + inttostr(swapCount);
end;
...
begin
  ...
  begin
    arr[i] := Random(51) - 25;
    StringGrid1.Cells[i - 1, 0] := inttostr(arr[i]); // выводим в string grid
    if (arr[i] > z) then
    begin
      StringGrid3.Cells[i - 1, 0] := inttostr(z);
      Inc(swapCount);
  ...
  Label3.Caption := 'Ответ: сумма элементов массива кратных 6 : ' +
    inttostr(sum);
  Label5.Caption := 'Количество замен: ' + inttostr(swapCount);
end;
